...
import com.hojune.getinline.dto.EventResponse;
import com.hojune.getinline.service.EventService;
...
class APIEventControllerTest {
    ...
    void givenParameters_whenRequestingEvents_thenReturnsListOfEventsInStandardResponse() throws Exception {
        // Given
        given(eventService.getEvents(any(), any(), any(), any(), any())).willReturn(List.of(creatEventDto()));
    ...
    void givenEvent_whenCreatingAnEvent_thenReturnsSuccessfulStandardResponse() throws Exception {
        ...
        given(eventService.creatEvent(any())).willReturn(true);
        ...
        mvc.perform(
                ...
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));
        then(eventService).should().creatEvent(any());
    }
    ...
    void givenEventId_whenRequestingExistentEvent_thenReturnsEventInStandardResponse() throws Exception {
        // Given
        long eventId = 1L;
        given(eventService.getEvent(eventId)).willReturn(Optional.of(creatEventDto()));
        ...
        mvc.perform(get("/api/events/" + eventId))
                ...
                .andExpect(jsonPath("$.data.memo").value("마스크를 꼭 착용하세요"))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));
    ...
    private EventDTO creatEventDto() {
        return EventDTO.of(
                1L,
                "오후 운동",
                EventStatus.OPENED,
